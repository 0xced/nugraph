...
public sealed partial class GlobalTool : IAsyncLifetime
{
    ...
    {
        await RunDotnetAsync(_workingDirectory, ["tool", "uninstall", "--global", "nugraph"]);
        if (_previousVersion != null)
        {
            await RunDotnetAsync(_workingDirectory, ["tool", "install", "--global", "nugraph", "--version", _previousVersion]);
        }
    ...
    {
        var installed = await RunDotnetAsync(_workingDirectory, ["tool", "list", "--global", "nugraph"], allowNonZeroExitCode: false);
        ...
        await RunDotnetAsync(_workingDirectory, installArgs);
    }

    private static async Task<string> RunDotnetAsync(DirectoryInfo workingDirectory, string[] arguments, bool allowNonZeroExitCode = false)
    {
